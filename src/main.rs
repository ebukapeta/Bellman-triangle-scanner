    ...
    pub async fn start_collection(&self, duration_secs: u64) -> ScanSummary {
        ...
        match connect_result {
            Ok(Ok((ws_stream, _))) => {
                ...
                // Bybit requires subscription to specific topics
                ...
                let subscribe_timeout = Instant::now() + Duration::from_secs(5);

                while Instant::now() < subscribe_timeout {
                ...
                if !subscribed {
                    ...
                    add_log(&mut logs, ScanLog {
                        timestamp: Local::now().format("%H:%M:%S").to_string(),
                        exchange: "bybit".to_string(),
                        message: "Subscription confirmation not received, continuing anyway".to_string(),
                    ...
                    match msg {
                        Ok(Message::Text(text)) => {
                            if let Ok(msg_data) = serde_json::from_str::<serde_json::Value>(&text) {
                                ...
                                if msg_data.get("success").is_some() {
                                    continue;
                                }

                                // Bybit v5 format: topic contains "tickers."
                                    ...
                                    if topic.starts_with("tickers.") {
                                        if let Some(data) = msg_data.get("data") {
                                            ...
                                            let bid = data.get("bid1Price")
                                                ...
                                                .or_else(|| data.get("bid1Price").and_then(|b| b.as_f64()));

                                            let ask = data.get("ask1Price")
                                                .or_else(|| data.get("askPrice"))
                                                .and_then(|a| a.as_str())
                                                .and_then(|s| s.parse::<f64>().ok())
                                                .or_else(|| data.get("ask1Price").and_then(|a| a.as_f64()));
        ...
        match connect_result {
            Ok(Ok((ws_stream, _))) => {
                ...
                while let Ok(Some(msg)) = timeout(Duration::from_secs(5), read.next()).await {
                    ...
                    // Send ping at required interval (usually 18-20 seconds)
                    ...
                    match msg {
                        Ok(Message::Text(text)) => {
                            if let Ok(msg_data) = serde_json::from_str::<serde_json::Value>(&text) {
                                ...
                                if msg_type == Some("message") || msg_type == Some("trade") {
                                    if let Some(data) = msg_data.get("data") {
                                        // KuCoin uses various formats, try multiple field names
                                        let symbol = data.get("symbol")
                                            .and_then(|s| s.as_str());
                                        ...
                                        let best_bid = data.get("bestBid")
                                            ...
                                            .and_then(|b| b.as_str())
                                            .and_then(|s| s.parse::<f64>().ok())
                                            .or_else(|| data.get("bestBid").and_then(|b| b.as_f64()));

                                        let best_ask = data.get("bestAsk")
                                            .or_else(|| data.get("ask"))
                                            .and_then(|a| a.as_str())
                                            .and_then(|s| s.parse::<f64>().ok())
                                            .or_else(|| data.get("bestAsk").and_then(|a| a.as_f64()));
...
async fn main() -> std::io::Result<()> {
    // Simple println logging instead of env_logger for cleaner output
