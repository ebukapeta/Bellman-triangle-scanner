...
impl ArbitrageDetector {
    ...
    // PHASE 2: Validate triangle by checking ORDER BOOK at market price
    ...
    fn validate_triangle_with_orderbook(&self, path: &[String], tickers: &HashMap<String, TickerData>) -> (f64, bool) {
        ...
        for i in 0..path.len() - 1 {
            ...
            for (symbol, data) in tickers {
                if let Some((base, quote)) = parse_symbol(symbol) {
                    ...
                    // Check: Can we sell at market price? (Is there a BID at or near market price?)
                    if &base == from && &quote == to {
                        trade_details = Some(("SELL", data));
                        break;
                    }
                    // Case 2: Buying 'to' (base) with 'from' (quote)
                    // Check: Can we buy at market price? (Is there an ASK at or near market price?)
            ...
            // ORDER BOOK VALIDATION AT MARKET PRICE
            let (execution_price, can_execute) = match action {
                "SELL" => {
                    // We want to sell at market price
                    // Check if best_bid exists and is close to market_price
                    let bid_available = data.best_bid > 0.0;
                    let price_close = (data.market_price - data.best_bid).abs() / data.market_price < 0.01; // Within 1%

                    // Check if there's quantity in order book
                    let has_liquidity = data.bid_depth.iter().any(|entry| entry.quantity > 0.0);

                    let can_exec = bid_available && price_close && has_liquidity;
                    if !can_exec { executable = false; }

                    (data.best_bid, can_exec)
                }
                "BUY" => {
                    // We want to buy at market price
                    // Check if best_ask exists and is close to market_price
                    let ask_available = data.best_ask > 0.0;
                    let price_close = (data.best_ask - data.market_price).abs() / data.market_price < 0.01; // Within 1%

                    // Check if there's quantity in order book
                    let has_liquidity = data.ask_depth.iter().any(|entry| entry.quantity > 0.0);

                    let can_exec = ask_available && price_close && has_liquidity;
                    if !can_exec { executable = false; }

                    (data.best_ask, can_exec)
                }
        ...
        // Only executable if profit is positive AND all legs have order book liquidity
            ...
            for &b_idx in &a_neighbors {
                ...
                for &c_idx in &b_neighbors {
                    ...
                    if valid_triangles <= 3 {
                        println!("🔍 Triangle {:?}: profit={:.2}%, executable={}", path, profit, is_executable);
                    }
