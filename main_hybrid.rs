...
impl ArbitrageDetector {
    ...
    // Returns (profit, is_executable)
        // CORRECTED: Use actual bid/ask prices for realistic profit calculation
        // User's requirement: Check if order exists AT MARKET PRICE, execute at market price
        // HYBRID: Market price + Order book validation
    // Check if market price exists in the CORRECT side of order book
    fn validate_triangle_with_orderbook(&self, path: &[String], tickers: &HashMap<String, TickerData>) -> (f64, bool) {
        ...
        for i in 0..path.len() - 1 {
            ...
            let (action, symbol, data) = match trade_details {
                Some(d) => d,
                None => {
            ...
            let market_price_executable = match action {
                "SELL" => {
                    // SELL: Check if market price exists in BID order book (buyers)
                    // We need someone willing to buy at our target price
                    let bid_at_market = data.bid_depth.iter().any(|entry| {
                        (entry.price - market_price).abs() / market_price < 0.001 // Within 0.1%
                    });

                    // Also check best bid is close to market
                    let best_bid_close = (market_price - data.best_bid).abs() / market_price < 0.005; // Within 0.5%

                    if !bid_at_market && !best_bid_close {
                        println!("  ❌ {}: No bid at market price {:.6} (best bid: {:.6})", 
                            symbol, market_price, data.best_bid);
                        executable = false;
                    }
                ...
                "BUY" => {
                    // BUY: Check if market price exists in ASK order book (sellers)
                    // We need someone willing to sell at our target price
                    let ask_at_market = data.ask_depth.iter().any(|entry| {
                        (entry.price - market_price).abs() / market_price < 0.001 // Within 0.1%
                    });

                    // Also check best ask is close to market
                    let best_ask_close = (data.best_ask - market_price).abs() / market_price < 0.005; // Within 0.5%

                    if !ask_at_market && !best_ask_close {
                        println!("  ❌ {}: No ask at market price {:.6} (best ask: {:.6})", 
                            symbol, market_price, data.best_ask);
                        executable = false;
                    }
            ...
            // Use MARKET PRICE for profit calculation (if validated)
            amount *= market_price;
            trade_count += 1;
        }
        ...
        // Only return if market price was executable on all legs
