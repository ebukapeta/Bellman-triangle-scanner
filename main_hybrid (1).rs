            ...
            Ok(Ok((mut ws_stream, _))) => {
                                        ...
                                        for item in arr {
                                            ...
                                            if let (Some(sym), Some(market), Some(bid), Some(ask)) = (symbol, last_price, bid_price, ask_price) {
                                                if parse_symbol(sym).is_some() && market > 0.0 && bid > 0.0 && ask > 0.0 {
                                                    ...
                                                    // FIX #5: Only count truly new pairs for logging accuracy
                                                    let is_new = !data.contains_key(sym);
                                                    data.insert(sym.to_string(), ticker_data);

                                                    if is_new {
                                                        let pair_count = data.len();
                                                        drop(data);
                                                        if pair_count % 500 == 0 {
                                                            let mut logs = logs_clone.lock().await;
                                                            add_log(&mut logs, ScanLog {
                ...
                let final_count = data_clone.lock().await.len();
                {
                    ...
                    add_log(&mut logs, ScanLog {
                        timestamp: Local::now().format("%H:%M:%S").to_string(),
                        exchange: "binance".to_string(),
                        message: format!("Collection complete: {} unique pairs", final_count),
                        level: "success".to_string(),
                    });
                }
            }
                                    ...
                                    if let Some(data) = msg_data.get("data") {
                                        ...
                                        if let (Some(symbol), Some(last), Some(bid), Some(ask)) = (symbol, last_price, best_bid, best_ask) {
                                            ...
                                            if parse_symbol(&std_symbol).is_some() && last > 0.0 && bid > 0.0 && ask > 0.0 {
                                                ...
                                                // FIX #6: Only count genuinely new pairs for KuCoin too
                                                let is_new = !data_map.contains_key(&std_symbol);
                                                ...
                                                if is_new {
                                                    ...
                                                    if count % 100 == 0 {
                                                        ...
                                                        add_log(&mut logs, ScanLog {
                                                            timestamp: Local::now().format("%H:%M:%S").to_string(),
                                                            exchange: "kucoin".to_string(),
                                                            message: format!("Collected {} unique pairs", count),
                                                            level: "debug".to_string(),
                                                        });
                                                    }
                                                }
...
impl ArbitrageDetector {
    ...
    fn build_graph(&self, tickers: &HashMap<String, TickerData>) -> (DiGraph<String, f64>, HashMap<String, NodeIndex>) {
        ...
        // FIX #7: Use a separate best_edges map keyed by symbol string to preserve
        // distinct trading pairs, only deduplicating true duplicates of the same symbol.
        // We still keep the best-weight edge per directed currency pair to avoid
        // multiple edges between the same two nodes (petgraph handles multi-edges
        // poorly in cycle detection), but this is now a documented design decision.
    ...
    // PHASE 2: Strict order book + BUY/SELL direction validation
    // FIX #1: BUY legs now correctly DIVIDE amount by price (not multiply)
    // FIX #2: Fee is applied per-leg as compounding during loop, not after on a potentially partial count
    // FIX #3: Amount update is gated — only happens when the leg passes order book validation
    // FIX #4 & #11: Unified staleness threshold (5s) with abs() guard against clock skew
    fn validate_triangle_with_orderbook(&self, path: &[String], tickers: &HashMap<String, TickerData>) -> (f64, bool) {
        ...
        let fee_rate = 0.001_f64;   // 0.1% fee per trade
        ...
        for i in 0..path.len() - 1 {
            ...
            let mut trade_details: Option<(&str, &TickerData)> = None;

            // Find the ticker and determine trade direction
            for (symbol, data) in tickers {
                if let Some((base, quote)) = parse_symbol(symbol) {
                    if &base == from && &quote == to {
                        // SELL base for quote
                        trade_details = Some(("SELL", data));
                        break;
                    } else if &base == to && &quote == from {
                        // BUY base (to) using quote (from)
                        trade_details = Some(("BUY", data));
                        break;
                    }
                }
            }

            let (action, data) = match trade_details {
                Some(d) => d,
                None => {
                    println!("  ❌ No ticker for {} -> {}", from, to);
                    return (-100.0, false);
                }
            };

            // FIX #11: Use abs() to guard against clock skew producing negative age
            let age_ms = (Utc::now().timestamp_millis() - data.timestamp).abs();
            // FIX #4: Consistent 5-second staleness threshold in both validation functions
            if age_ms > 5000 {
                return (-100.0, false);
            }

            let market_price = data.market_price;

            // HYBRID LOGIC: Check if market price exists in correct order book side
            let market_price_executable = match action {
                "SELL" => {
                    // SELL base for quote: need a buyer in the BID book at/near market price
                    let bid_at_market = data.bid_depth.iter().any(|entry| {
                        entry.price > 0.0
                            && (entry.price - market_price).abs() / market_price < 0.001
                    });
                    let best_bid_close = data.best_bid > 0.0
                        && (market_price - data.best_bid).abs() / market_price < 0.005;

                    if !bid_at_market && !best_bid_close {
                        println!("  ❌ SELL {}: No bid at market price {:.6} (best bid: {:.6})",
                            from, market_price, data.best_bid);
                        executable = false;
                    }

                    bid_at_market || best_bid_close
                }
                "BUY" => {
                    // BUY base with quote: need a seller in the ASK book at/near market price
                    let ask_at_market = data.ask_depth.iter().any(|entry| {
                        entry.price > 0.0
                            && (entry.price - market_price).abs() / market_price < 0.001
                    });
                    let best_ask_close = data.best_ask > 0.0
                        && (data.best_ask - market_price).abs() / market_price < 0.005;

                    if !ask_at_market && !best_ask_close {
                        println!("  ❌ BUY {}: No ask at market price {:.6} (best ask: {:.6})",
                            to, market_price, data.best_ask);
                        executable = false;
                    }

                    ask_at_market || best_ask_close
                }
                _ => false,
            };

            // FIX #3: Only update amount when this leg is actually executable
            if market_price_executable {
                // FIX #1: BUY legs divide by price; SELL legs multiply by price
                // FIX #2: Apply fee per-leg (compounding) instead of once at the end
                match action {
                    "SELL" => amount *= market_price * (1.0 - fee_rate),
                    "BUY"  => amount = amount / market_price * (1.0 - fee_rate),
                    _ => {}
                }
            } else {
                executable = false;
        ...
        // Net profit as a percentage gain/loss over the starting amount of 1.0
        let net_profit = (amount - 1.0) * 100.0;

        (net_profit, executable && net_profit > 0.0)
    }
        ...
        for i in 0..path.len() - 1 {
            ...
            for (symbol, data) in tickers {
                if let Some((base, quote)) = parse_symbol(symbol) {
                    ...
                    if matches {
                        // FIX #4: Use the same 5s staleness threshold as validate_triangle_with_orderbook
                        // FIX #11: abs() to guard against clock skew
                        let age_ms = (Utc::now().timestamp_millis() - data.timestamp).abs();
                        let freshness = if age_ms < 500 { 100.0 }
                            else if age_ms < 2000 { 90.0 }
                            else if age_ms < 5000 { 70.0 }
                            else { 0.0 };   // Beyond 5s is stale — score zero, consistent with validator

                        // Price alignment score (market price vs bid/ask)
                        let spread = if data.market_price > 0.0 {
                            ((data.best_ask - data.best_bid) / data.market_price * 100.0).abs()
                        } else {
                            100.0
                        };
    ...
    fn find_opportunities(&self, graph: &DiGraph<String, f64>, node_indices: &HashMap<String, NodeIndex>,
        ...
        for &a_idx in &nodes {
            ...
            for &b_idx in &a_neighbors {
                ...
                for &c_idx in &b_neighbors {
                    ...
                    // Only surface opportunities that are profitable AND order-book validated
        ...
        // FIX #12: Deduplicate both rotations AND reverse-direction cycles
        let mut seen: HashSet<String> = HashSet::new();
        opportunities.retain(|opp| {
            let mut cycle = opp.triangle.clone();
            cycle.pop(); // remove the closing node (same as first)

            // Generate the canonical key for both the forward and reverse directions,
            // then keep whichever string is lexicographically smaller so that
            // A→B→C and A→C→B are treated as the same opportunity.
            let canonical_forward = {
                let min_pos = cycle.iter().enumerate()
                    .min_by_key(|(_, x)| x.as_str())
                    .map(|(i, _)| i)
                    .unwrap_or(0);
            ...
            rev_cycle.reverse();
            let canonical_reverse = {
                let min_pos = rev_cycle.iter().enumerate()
                    .min_by_key(|(_, x)| x.as_str())
                    .map(|(i, _)| i)
                    .unwrap_or(0);
                let mut rotated = rev_cycle[min_pos..].to_vec();
                rotated.extend_from_slice(&rev_cycle[..min_pos]);
                rotated.join(",")
            };

            // Use the lexicographically smaller key so forward and reverse share one slot
            let key = canonical_forward.min(canonical_reverse);

            if seen.contains(&key) { false } else { seen.insert(key); true }
        });

        opportunities.sort_by(|a, b| b.profit_margin_before.partial_cmp(&a.profit_margin_before)
    ...
    pub async fn scan_exchange(&self, exchange: &str, min_profit: f64, duration_secs: u64)
       ...
       // FIX #8: Yield to the async runtime periodically inside find_opportunities
       // by running it in a blocking thread pool so it doesn't starve other tasks.
       let tickers_clone = tickers.clone();
       let node_indices_clone = node_indices.clone();
       // We move graph and tickers into a spawn_blocking closure
       // so the O(N³) loop does not block the Tokio runtime thread.
       let detector_snapshot = ArbitrageDetectorSnapshot {
           tickers: tickers_clone,
           node_indices: node_indices_clone,
           min_profit,
       };
       ...
       let (opportunities, paths_checked, valid_triangles, profitable) =
           tokio::task::spawn_blocking(move || {
               detector_snapshot.find_opportunities_blocking(&graph_clone)
           })
           .await
           .unwrap_or_else(|_| (Vec::new(), 0, 0, 0));
    ...
    pub async fn scan_multiple_exchanges(&self, exchanges: Vec<String>, min_profit: f64, duration_secs: u64)
        ...
        all_opportunities.sort_by(|a, b| b.profit_margin_before.partial_cmp(&a.profit_margin_before)
            .unwrap_or(std::cmp::Ordering::Equal));
...
// ==================== Snapshot struct for spawn_blocking (FIX #8) ====================
// Carries cloned data into the blocking thread so the O(N³) loop
// does not block the Tokio async runtime thread.

struct ArbitrageDetectorSnapshot {
    tickers: HashMap<String, TickerData>,
    node_indices: HashMap<String, NodeIndex>,
    min_profit: f64,
}

impl ArbitrageDetectorSnapshot {
    fn validate_triangle_with_orderbook(&self, path: &[String]) -> (f64, bool) {
        if path.len() < 3 || path.first() != path.last() {
            return (-100.0, false);
        }
        ...
        for i in 0..path.len() - 1 {
            ...
            for (symbol, data) in &self.tickers {
                if let Some((base, quote)) = parse_symbol(symbol) {
                    if &base == from && &quote == to {
                        trade_details = Some(("SELL", data));
                        break;
            ...
            // FIX #11: abs() against clock skew; FIX #4: consistent 5s threshold
            let age_ms = (Utc::now().timestamp_millis() - data.timestamp).abs();
            if age_ms > 5000 {
                return (-100.0, false);
            }
            ...
            // FIX #3: Gate amount update on executability
            if market_price_executable {
                // FIX #1 + #2: Correct direction + per-leg fee
                match action {
                    "SELL" => amount *= market_price * (1.0 - fee_rate),
                    "BUY"  => amount = amount / market_price * (1.0 - fee_rate),
                    _ => {}
        ...
        for i in 0..path.len() - 1 {
            ...
            for (symbol, data) in &self.tickers {
                if let Some((base, quote)) = parse_symbol(symbol) {
                    ...
                    if matches {
                        // FIX #4 + #11: consistent staleness with abs() guard
                        let age_ms = (Utc::now().timestamp_millis() - data.timestamp).abs();
                        let freshness = if age_ms < 500 { 100.0 }
                            else if age_ms < 2000 { 90.0 }
                            else if age_ms < 5000 { 70.0 }
    ...
    fn find_opportunities_blocking(&self, graph: &DiGraph<String, f64>)
        -> (Vec<ArbitrageOpportunity>, usize, usize, usize) {
        ...
        for &a_idx in &nodes {
            ...
            for &b_idx in &a_neighbors {
                ...
                for &c_idx in &b_neighbors {
                    ...
                    let (profit, is_executable) = self.validate_triangle_with_orderbook(&path);

                    if valid_triangles <= 3 {
                        println!("🔍 Triangle {:?}: profit={:.4}%, executable={}", path, profit, is_executable);
                    }

                    if profit > self.min_profit && profit < 50.0 && profit.is_finite() && is_executable {
