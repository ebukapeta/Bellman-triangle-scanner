...
impl ArbitrageDetector {
    ...
    fn validate_triangle_with_orderbook(&self, path: &[String], tickers: &HashMap<String, TickerData>) -> (f64, bool) {
        ...
        for i in 0..path.len() - 1 {
            ...
            // USER'S REQUIREMENT: Check if order book has order AT MARKET PRICE
            let market_price = data.market_price;
            let can_execute_at_market = match action {
                "SELL" => {
                    // Selling: Check if there's a BID at or near market price
                    // (Someone willing to buy at the last traded price)
                    let bid_available = data.best_bid > 0.0;
                    let price_match = (market_price - data.best_bid).abs() / market_price < 0.005; // Within 0.5%
                    let has_quantity = data.bid_depth.iter().any(|e| e.quantity > 0.0);

                    bid_available && price_match && has_quantity
                }
                "BUY" => {
                    // Buying: Check if there's an ASK at or near market price
                    // (Someone willing to sell at the last traded price)
                    let ask_available = data.best_ask > 0.0;
                    let price_match = (data.best_ask - market_price).abs() / market_price < 0.005; // Within 0.5%
                    let has_quantity = data.ask_depth.iter().any(|e| e.quantity > 0.0);

                    ask_available && price_match && has_quantity
                }
                _ => false,
            };

            if !can_execute_at_market {
                executable = false;
            }

            // Execute at MARKET PRICE (not bid/ask)
            amount *= market_price;
            trade_count += 1;
        }
        ...
        // Only return if executable at market price AND profitable
